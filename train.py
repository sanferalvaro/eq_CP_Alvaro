...
if __name__ == "__main__":
    ...
    parser.add_argument("--analysis"     , type=str, default="ttbar", choices=['ttbar','ttbar_ideal','ttbar_withneutrinos', 'ttbb_godmode', 'ttZ_3l','ttZ_3l_v2','ttA_1l','ttW', 'ttbar_pl', 'ttbar_pl_sl','ttA_pl', 'ww', 'wz','tzq_pl', 'ttz_pl', 'ttA_delphes', 'wz_delphes'], help="Analysis to run, defines dataset type and neural network")
    ...
    if args.analysis == 'ttbar':
        ...
        if bool(args.noequivariant):
           ...
           print("equivariant")
    ###NUEVO###
    elif args.analysis == 'ttbar_pl_sl':
        from data_networks_ttbar_semilepton_particle_level import dataset
        if bool(args.noequivariant):
           from data_networks_ttbar_semilepton_particle_level import network_noeq as network
           print("no equivariant")
        else:
           from data_networks_ttbar_semilepton_particle_level import network
           print("equivariant")
    ###NUEVO###
    ...
    print("Loading test ")
    test     =dataset( f'{data_path}/test/*.{args.data_format}', device=args.device)
    print(f"Torch is using device: {args.device}") #nuevo
...
"""
    ...
    for ep in range(args.epochs):
        ...
        for weight, control, input_vars in loop:
            ...
            optimizer.zero_grad()
            score=net( input_vars ).to(args.device) #nuevo
        ...
        with torch.no_grad():
            ...
            def do_end_of_era_processing( dataset, name ):
                ...
                for weight, control, input_vars in dataset:
                    ...
                    weight=weight.to(args.device)#nuevo
